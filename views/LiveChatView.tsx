  ...
  const renderContent = () => {
    switch (message.message_type) {
      ...
      case 'order': {
        ...
        const orderId = (meta.order_id as string | number) ?? '—';
        const status = (meta.status as string) ?? 'pending';
        const total = (meta.total as string) ?? '';
        return (
          <div className="space-y-1">
            <div className="text-xs uppercase tracking-wide opacity-75">Order</div>
            <div className="font-semibold">#{orderId}</div>
            <div className="text-sm">{status}{total ? ` · ${total}` : ''}</div>
            {message.content && message.content !== '[order]' && (
              <div className="pt-1 text-sm">{message.content}</div>
            )}
          </div>
        );
      }
      case 'product': {
        ...
        const productId = meta.product_id as string | number | undefined;
        return (
          <div className="space-y-1">
            <div className="text-xs uppercase tracking-wide opacity-75">Product</div>
            <div className="font-semibold">{name}</div>
            {price && <div className="text-sm">{price}</div>}
            {productId !== undefined && (
              <div className="text-xs opacity-75">id: {productId}</div>
            )}
          </div>
        );
      }
...
export default function LiveChatView({ onBack }: LiveChatViewProps) {
  ...
  // Auto-scroll to the bottom whenever messages change. Only when the
  // user is already near the bottom — otherwise we'd interrupt them
  // scrolling up to read history. Threshold: 100px from bottom.
