...
const ShopView: React.FC<ShopViewProps> = ({ isLoggedIn, user, onLoginRequest, onPurchaseComplete, simType, onSwitchToMySims, hasActiveSims, onSwitchToSetup, onAddCard, onNavigate, onSwitchSimType, notifications = [] }) => {
  ...
  // Filter eSIM groups by search + continent tab
  const filteredEsimGroups = esimGroups.filter(g => {
    ...
    const matchesTab = searchQuery || continentTab === 'All' ||
      (continentTab === 'Multi-Region' ? g.isMultiRegion : (!g.isMultiRegion && g.continent === continentTab));
  ...
  return (
    <div className="sm:h-full relative bg-[#F2F4F7]">
      <div ref={scrollContainerRef} className="h-full sm:overflow-y-auto no-scrollbar">
        ...
        <div
          className={`px-4 pt-safe pb-3 sticky top-0 z-40 border-b transition-all duration-300 ease-out ${headerHidden ? 'bg-transparent border-transparent' : 'bg-white border-slate-100'}`}
                    ...
                    {CONTINENT_TABS.map((tab) => (
                      ...
                      >
                        {tab === 'All' ? t('shop.all_regions')
                          : tab === 'Multi-Region' ? t('shop.multi_region')
