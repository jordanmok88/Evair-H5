...
const PhysicalSimSetupView: React.FC<PhysicalSimSetupViewProps> = ({
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#F2F4F7] lg:h-full">
      ...
      <div
        className={
          activateStep === 'SCAN'
            ? 'flex-1 overflow-y-auto no-scrollbar px-4 pb-3 pb-safe-bottom min-h-0'
            : 'flex-1 overflow-y-auto no-scrollbar px-4 pb-6'
        }
      >

        <div className={activateStep === 'SCAN' ? 'pt-2' : 'pt-4'}>

          {/* ── Step Progress Indicator ── */}
          <div
            className={`bg-white rounded-2xl shadow-sm border border-slate-100 mb-2 ${
              activateStep === 'SCAN' ? 'p-2.5' : 'p-4 mb-4'
            }`}
          >
            <div className="flex items-center justify-between">
              {(['SCAN', 'CONFIRM', 'DONE'] as const).map((step, i) => {
                ...
                const scanCompact = activateStep === 'SCAN';
                const iconWrap = scanCompact ? 'w-7 h-7' : 'w-9 h-9';
                const iconSz = scanCompact ? 14 : 16;
                return (
                  <React.Fragment key={step}>
                    {i > 0 && (
                      <div className={`flex-1 h-0.5 mx-1 rounded-full ${isDone ? 'bg-brand-orange' : 'bg-slate-200'}`} />
                    )}
                    <div className={`flex flex-col items-center ${scanCompact ? 'gap-0.5' : 'gap-1'}`}>
                      <div className={`${iconWrap} rounded-full flex items-center justify-center transition-all ${
                        isDone ? 'bg-brand-orange text-white' :
                        isActive ? 'bg-orange-50 text-brand-orange ring-2 ring-brand-orange' :
                        'bg-slate-100 text-slate-400'
                      }`}>
                        {isDone ? <Check size={iconSz} /> : <StepIcon size={iconSz} />}
                      </div>
                      <span className={`${scanCompact ? 'text-[10px]' : 'text-[11px]'} font-bold ${isActive || isDone ? 'text-brand-orange' : 'text-slate-400'}`}>
                        {stepLabelsMap[i]}
                      </span>
                    </div>
                  </React.Fragment>
          ...
          {/* ── Step 1: SCAN ── */}
          {activateStep === 'SCAN' && (
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-3 mb-3 [@media(min-height:740px)]:p-4">
              {/* "Your SIM already works" reassurance — critical because
                  ...
                  gating action. */}
              <div className="bg-green-50 border border-green-100 rounded-lg px-2.5 py-2 mb-2 flex items-start gap-2">
                <Info size={14} className="text-green-600 shrink-0 mt-0.5" />
                <p className="text-[11px] text-green-800 leading-snug [@media(min-height:740px)]:text-[12px]">
                  {t(
              ...
              <div className="flex justify-between items-start gap-2 mb-2">
                <div className="min-w-0">
                  <h3 className="text-sm font-bold text-slate-900 [@media(min-height:740px)]:text-[15px]">{t('sim_setup.bind_title')}</h3>
                  <p className="text-xs text-slate-400 mt-0.5 leading-snug [@media(min-height:740px)]:text-[13px]">{t('sim_setup.bind_scan_desc')}</p>
                </div>
                <div className="w-8 h-8 bg-orange-50 rounded-lg flex items-center justify-center shrink-0 [@media(min-height:740px)]:w-9 [@media(min-height:740px)]:h-9 [@media(min-height:740px)]:rounded-xl">
                  <Smartphone className="h-4 w-4 [@media(min-height:740px)]:h-[18px] [@media(min-height:740px)]:w-[18px] text-brand-orange" />
                </div>
              </div>

              <div className="border-2 border-brand-orange rounded-xl h-11 [@media(min-height:740px)]:h-12 flex items-center px-3 bg-white shadow-sm mb-2">
                <input
                  type="text"
                  value={iccidInput}
                  onChange={(e) => setIccidInput(e.target.value.replace(/[^0-9]/g, ''))}
                  placeholder="ICCID / EID Number"
                  className="flex-1 h-full outline-none text-slate-900 font-semibold placeholder:text-slate-400 placeholder:font-normal bg-transparent text-[15px] tracking-wider"
                  maxLength={22}
                />
                <button onClick={() => setScannerOpen(true)} className="p-1.5 -mr-1 text-slate-400 hover:text-brand-orange transition-colors">
                  <ScanLine size={20} />
                </button>
              </div>

              {/* Retail packaging — height-capped on SCAN so field + CTA stay one viewport. */}
              <div className="-mx-3 mb-2 [@media(min-height:740px)]:-mx-4">
                <SimCardIccidHint compact />
              </div>
              ...
              <button
                ...
                className="w-full py-3 rounded-xl font-bold text-sm [@media(min-height:740px)]:py-3.5 [@media(min-height:740px)]:text-[15px] flex items-center justify-center gap-2 active:scale-[0.98] transition-all"
                style={{
