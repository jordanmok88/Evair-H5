    ...
    <div className="flex h-full min-h-0 flex-col bg-[#F2F4F7] lg:h-full">
      ...
      >
        ...
        >
          ...
          <div
            className={`bg-white shrink-0 rounded-2xl border border-slate-100 shadow-sm ${
              activateStep === 'SCAN' ? 'p-3' : 'mb-4 p-4'
            }`}
          >
            <div className="flex items-center justify-between">
              {(['SCAN', 'CONFIRM', 'DONE'] as const).map((step, i) => {
                ...
                const scanLayout = activateStep === 'SCAN';
                const iconWrap = scanLayout ? 'h-9 w-9' : 'h-9 w-9';
                const iconSz = scanLayout ? 16 : 16;
                return (
                  <React.Fragment key={step}>
                    {i > 0 && (
                      <div className={`mx-1.5 h-0.5 flex-1 rounded-full ${isDone ? 'bg-brand-orange' : 'bg-slate-200'}`} />
                    )}
                    <div className={`flex flex-col items-center ${scanLayout ? 'gap-1' : 'gap-1'}`}>
                      <div className={`${iconWrap} flex items-center justify-center rounded-full transition-all ${
            ...
            <div className="flex min-h-0 flex-1 basis-0 flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
              <div className="shrink-0 space-y-3 p-3 sm:p-4">
                <div className="flex items-start gap-2 rounded-lg border border-green-100 bg-green-50 px-3 py-2.5">
                  ...
                  <p className="text-xs leading-snug text-green-800 sm:text-[13px]">
                    {t(
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    <h3 className="text-[15px] font-bold leading-tight text-slate-900">{t('sim_setup.bind_title')}</h3>
                    <p className="mt-1 text-[13px] leading-snug text-slate-500">{t('sim_setup.bind_scan_desc')}</p>
                  </div>
                ...
                <div className="flex h-12 items-center rounded-xl border-2 border-slate-200 bg-white px-3 shadow-sm transition-[box-shadow,border-color] focus-within:border-brand-orange focus-within:ring-2 focus-within:ring-brand-orange/20">
                  <input
                    type="text"
                    value={iccidInput}
                    onChange={(e) => setIccidInput(e.target.value.replace(/[^0-9]/g, ''))}
                    placeholder="ICCID / EID Number"
                    className="h-full flex-1 bg-transparent text-[16px] font-semibold tracking-wider text-slate-900 outline-none placeholder:font-normal placeholder:text-slate-400"
                    maxLength={22}
                  />
                  <button
                    type="button"
                    onClick={() => setScannerOpen(true)}
                    className="-mr-1 p-2 text-slate-400 transition-colors hover:text-brand-orange"
                  >
                    <ScanLine size={22} />
                  </button>
                </div>
              ...
              <div className="flex min-h-0 flex-1 flex-col px-3 pb-3 pt-1 sm:px-4 sm:pb-4">
                ...
                className="mt-3 flex w-full shrink-0 items-center justify-center gap-2 rounded-xl py-3.5 text-[15px] font-bold transition-all active:scale-[0.98]"
                style={{
              ...
              {activationError && (
                <div className="mt-3 flex shrink-0 items-start gap-2 rounded-xl border border-red-100 bg-red-50 px-3 py-2.5">
