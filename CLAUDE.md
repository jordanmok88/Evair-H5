...
- `.cursor/rules/jordan-chat-history.mdc` — **Jordan chat history** (distilled chat outcomes), updated **proactively** by assistants when Jordan states lasting prefs (no cue phrase needed).
...
**Agents:** After H5 code changes, run `npm run save` (or `bash scripts/save.sh "msg"`) **in the same turn** so **GitHub** updates — that is how Jordan gets **live Netlify** previews (typically ~1–3 min after webhook). Not a separate Netlify CLI step. If `origin` rejects, `git pull --rebase origin main` then push again (see `.cursor/rules/auto-save-workflow.mdc`).
