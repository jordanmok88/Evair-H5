...
function getAllowedOrigins(env2) {
  ...
  const extra = env2?.ALLOWED_ORIGINS;
  if (extra?.trim()) {
...
function resolveCors(req, env2) {
  const origin = req.headers.get("Origin");
  const allowed = getAllowedOrigins(env2);
  if (!origin) {
...
function stripeCheckoutLimits(env2) {
  return {
    name: "stripe-checkout",
    max: Number(env2?.RATE_LIMIT_STRIPE_MAX) || 20,
    windowMs: Number(env2?.RATE_LIMIT_STRIPE_WINDOW_MS) || 6e4
  };
}
__name(stripeCheckoutLimits, "stripeCheckoutLimits");
function sendEmailLimits(env2) {
  return {
    name: "send-esim-email",
    max: Number(env2?.RATE_LIMIT_EMAIL_MAX) || 8,
    windowMs: Number(env2?.RATE_LIMIT_EMAIL_WINDOW_MS) || 6e4
  };
}
__name(sendEmailLimits, "sendEmailLimits");
function esimProxyLimits(env2) {
  return {
    name: "esim-proxy",
    max: Number(env2?.RATE_LIMIT_ESIM_MAX) || 60,
    windowMs: Number(env2?.RATE_LIMIT_ESIM_WINDOW_MS) || 6e4
  };
}
__name(esimProxyLimits, "esimProxyLimits");
function trackLimits(env2) {
  return {
    name: "track",
    max: Number(env2?.RATE_LIMIT_TRACK_MAX) || 120,
    windowMs: Number(env2?.RATE_LIMIT_TRACK_WINDOW_MS) || 6e4
  };
}
__name(trackLimits, "trackLimits");
function stripeVerifyLimits(env2) {
  return {
    name: "stripe-verify",
    max: Number(env2?.RATE_LIMIT_VERIFY_MAX) || 40,
    windowMs: Number(env2?.RATE_LIMIT_VERIFY_WINDOW_MS) || 6e4
  };
}
...
function makeAuthHeaders(bodyString, env2) {
  const accessCode = env2.ESIM_ACCESS_CODE;
  const secret = env2.ESIM_SECRET;
...
async function proxyRequest(endpoint, body, env2) {
  const url = `${API_BASE}${endpoint}`;
  const bodyString = JSON.stringify(body);
  const headers = makeAuthHeaders(bodyString, env2);
...
async function onRequest(context2) {
  const { request: req, env: env2 } = context2;
  const { allowOrigin, rejectCrossOriginBrowser } = resolveCors(req, env2);
  ...
  if (rateLimitExceeded(req, esimProxyLimits(env2))) {
  ...
  try {
    ...
    const data = await proxyRequest(endpoint, payload ?? {}, env2);
...
// ../lib/cloudflare/sendEsimEmailHtml.ts
...
async function onRequest2(context2) {
  ...
  if (rateLimitExceeded(req, sendEmailLimits(env2))) {
    return new Response(JSON.stringify({ error: "Too many requests" }), {
      status: 429,
      headers
    });
  ...
  try {
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: env2.RESEND_FROM || "EvairSIM <onboarding@resend.dev>",
        to: [email],
...
async function onRequest3(context2) {
  ...
  if (rateLimitExceeded(req, stripeCheckoutLimits(env2))) {
    return new Response(JSON.stringify({ error: "Too many requests" }), {
      status: 429,
      headers
    });
  ...
  try {
    ...
    const isSafeReturn = /* @__PURE__ */ __name((url) => typeof url === "string" && url.length < 2048 && url.startsWith(callerOrigin), "isSafeReturn");
...
async function onRequest4(context2) {
  ...
  if (rateLimitExceeded(req, stripeVerifyLimits(env2))) {
    return new Response(JSON.stringify({ error: "Too many requests" }), {
      status: 429,
      headers
    });
...
async function onRequest5(context2) {
  ...
  if (rateLimitExceeded(req, trackLimits(env2))) {
    return new Response(JSON.stringify({ error: "Too many requests" }), {
      status: 429,
      headers
    });
...
async function onRequest6(context2) {
  ...
  const target = (env2.QR_SCAN_REDIRECT_URL || DEFAULT_PUBLIC_APP).replace(/\/$/, "");
...
//# sourceMappingURL=functionsWorker-0.5799136884439087.mjs.map
