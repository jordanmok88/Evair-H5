...
// .wrangler/tmp/pages-gKFjPb/functionsWorker-0.5799136884439087.mjs
...
var DEFAULT_ORIGINS = ["https://evairdigital.com", "https://www.evairdigital.com"];
function getAllowedOrigins(env22) {
  const set = new Set(DEFAULT_ORIGINS);
  const extra = env22?.ALLOWED_ORIGINS;
  if (extra?.trim()) {
...
function resolveCors(req, env22) {
  const origin = req.headers.get("Origin");
  const allowed = getAllowedOrigins(env22);
  if (!origin) {
...
__name(resolveCors, "resolveCors");
__name2(resolveCors, "resolveCors");
...
function ipFromReq(req) {
  return req.headers.get("cf-connecting-ip") || req.headers.get("x-nf-client-connection-ip") || req.headers.get("client-ip") || (req.headers.get("x-forwarded-for") || "").split(",")[0]?.trim() || "unknown";
}
__name(ipFromReq, "ipFromReq");
__name2(ipFromReq, "ipFromReq");
function rateLimitExceeded(req, { name, max, windowMs }) {
  const m = Number.isFinite(Number(max)) ? Number(max) : 40;
  const w = Number.isFinite(Number(windowMs)) ? Number(windowMs) : 6e4;
...
__name2(rateLimitExceeded, "rateLimitExceeded");
function stripeCheckoutLimits(env22) {
  return {
    name: "stripe-checkout",
    max: Number(env22?.RATE_LIMIT_STRIPE_MAX) || 20,
    windowMs: Number(env22?.RATE_LIMIT_STRIPE_WINDOW_MS) || 6e4
  };
}
__name(stripeCheckoutLimits, "stripeCheckoutLimits");
__name2(stripeCheckoutLimits, "stripeCheckoutLimits");
function sendEmailLimits(env22) {
  return {
    name: "send-esim-email",
    max: Number(env22?.RATE_LIMIT_EMAIL_MAX) || 8,
    windowMs: Number(env22?.RATE_LIMIT_EMAIL_WINDOW_MS) || 6e4
  };
}
__name(sendEmailLimits, "sendEmailLimits");
__name2(sendEmailLimits, "sendEmailLimits");
function esimProxyLimits(env22) {
  return {
    name: "esim-proxy",
    max: Number(env22?.RATE_LIMIT_ESIM_MAX) || 60,
    windowMs: Number(env22?.RATE_LIMIT_ESIM_WINDOW_MS) || 6e4
  };
}
__name(esimProxyLimits, "esimProxyLimits");
__name2(esimProxyLimits, "esimProxyLimits");
function trackLimits(env22) {
  return {
    name: "track",
    max: Number(env22?.RATE_LIMIT_TRACK_MAX) || 120,
    windowMs: Number(env22?.RATE_LIMIT_TRACK_WINDOW_MS) || 6e4
  };
}
__name(trackLimits, "trackLimits");
__name2(trackLimits, "trackLimits");
function stripeVerifyLimits(env22) {
  return {
    name: "stripe-verify",
    max: Number(env22?.RATE_LIMIT_VERIFY_MAX) || 40,
    windowMs: Number(env22?.RATE_LIMIT_VERIFY_WINDOW_MS) || 6e4
  };
}
__name(stripeVerifyLimits, "stripeVerifyLimits");
__name2(stripeVerifyLimits, "stripeVerifyLimits");
var API_BASE = "https://api.esimaccess.com/api/v1/open";
function makeAuthHeaders(bodyString, env22) {
  const accessCode = env22.ESIM_ACCESS_CODE;
  const secret = env22.ESIM_SECRET;
  ...
  const signature = createHmac("sha256", secret).update(signData).digest("hex").toUpperCase();
  return {
...
__name2(makeAuthHeaders, "makeAuthHeaders");
async function proxyRequest(endpoint, body, env22) {
  const url = `${API_BASE}${endpoint}`;
  const bodyString = JSON.stringify(body);
  const headers = makeAuthHeaders(bodyString, env22);
...
__name2(proxyRequest, "proxyRequest");
var ALLOWED_ENDPOINTS = ["/package/list", "/esim/order", "/esim/query", "/esim/topup", "/esim/usage"];
async function onRequest(context22) {
  const { request: req, env: env22 } = context22;
  const { allowOrigin, rejectCrossOriginBrowser } = resolveCors(req, env22);
  ...
  if (rateLimitExceeded(req, esimProxyLimits(env22))) {
  ...
  try {
    ...
    const data = await proxyRequest(endpoint, payload ?? {}, env22);
...
var SEND_ESIM_APP_ORIGIN = "https://evairdigital.com";
var BRAND_ORANGE = "#FF6600";
var BRAND_DARK = "#0F172A";
var SUPPORT_EMAIL = "support@evairdigital.com";
...
function buildSendEsimEmailHtml(p) {
  ...
  const manageHref = p.iccid ? `${SEND_ESIM_APP_ORIGIN}/app/my-sims?iccid=${encodeURIComponent(String(p.iccid))}` : `${SEND_ESIM_APP_ORIGIN}/app/my-sims`;
  const qrBlock = p.qrCodeUrl ? `<tr><td align="center" style="padding:0 24px 8px">
       ...
       </td></tr>` : `<tr><td align="center" style="padding:0 24px 16px">
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f2f4f7;padding:24px 12px">
    <tr><td align="center">
      ...
      <p style="text-align:center;font-size:11px;color:#94a3b8;margin:16px 0 0">
        &copy; ${(/* @__PURE__ */ new Date()).getFullYear()} EvairSIM &middot;
...
__name(buildSendEsimEmailHtml, "buildSendEsimEmailHtml");
__name2(buildSendEsimEmailHtml, "buildSendEsimEmailHtml");
async function onRequest2(context22) {
  ...
  if (rateLimitExceeded(req, sendEmailLimits(env22))) {
    return new Response(JSON.stringify({ error: "Too many requests" }), {
      status: 429,
      headers
    });
  ...
  try {
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: env22.RESEND_FROM || "EvairSIM <onboarding@resend.dev>",
        to: [email],
...
async function onRequest3(context22) {
  ...
  if (rateLimitExceeded(req, stripeCheckoutLimits(env22))) {
    return new Response(JSON.stringify({ error: "Too many requests" }), {
      status: 429,
      headers
    });
  }
  const secretKey = env22.STRIPE_SECRET_KEY;
  if (!secretKey) {
  ...
  try {
    ...
    if (!packageName || priceUsd === void 0 || priceUsd === null || !packageCode) {
      return new Response(
    ...
    const callerOrigin = allowOrigin || req.headers.get("Origin") || req.headers.get("Referer")?.split("/").slice(0, 3).join("/") || "https://evairdigital.com";
    const isSafeReturn = /* @__PURE__ */ __name2((url) => typeof url === "string" && url.length < 2048 && url.startsWith(callerOrigin), "isSafeReturn");
    const safeSuccessUrl = isSafeReturn(successUrl) ? successUrl : `${callerOrigin}/?stripe_status=success&session_id={CHECKOUT_SESSION_ID}`;
    ...
    const flagUrl = countryStr ? `https://wsrv.nl/?url=flagcdn.com/w320/${countryStr.toLowerCase()}.png&w=324&h=217&fit=contain&we&cbg=d0d3d8` : void 0;
    const orderMetadata = orderId != null && orderId !== "" ? {
      order_type: "order",
    ...
    const session = await stripe.checkout.sessions.create({
      mode: "payment",
      customer_email: typeof email === "string" ? email || void 0 : void 0,
...
async function onRequest4(context22) {
  ...
  if (rateLimitExceeded(req, stripeVerifyLimits(env22))) {
    return new Response(JSON.stringify({ error: "Too many requests" }), {
      status: 429,
      headers
    });
...
async function onRequest5(context22) {
  ...
  if (rateLimitExceeded(req, trackLimits(env22))) {
    return new Response(JSON.stringify({ error: "Too many requests" }), {
      status: 429,
      headers
    });
...
var DEFAULT_PUBLIC_APP = "https://www.evairdigital.com/app";
...
__name2(geoFromRequest, "geoFromRequest");
async function onRequest6(context22) {
  const { request, env: env22 } = context22;
  const url = new URL(request.url);
  const src = url.searchParams.get("src") || "direct";
  const target = (env22.QR_SCAN_REDIRECT_URL || DEFAULT_PUBLIC_APP).replace(/\/$/, "");
  const supabaseUrl = env22.SUPABASE_URL;
  const supabaseKey = env22.SUPABASE_SERVICE_KEY;
...
__name(parseDevice2, "parseDevice2");
__name2(parseDevice2, "parseDevice");
...
__name(geoFromRequest2, "geoFromRequest2");
__name2(geoFromRequest2, "geoFromRequest");
async function onRequest7(context22) {
  const { request, env: env22 } = context22;
  const url = new URL(request.url);
  const src = url.searchParams.get("src") || "direct";
  const target = (env22.QR_SCAN_REDIRECT_URL || DEFAULT_PUBLIC_APP2).replace(/\/$/, "");
  const supabaseUrl = env22.SUPABASE_URL;
  const supabaseKey = env22.SUPABASE_SERVICE_KEY;
  if (supabaseUrl && supabaseKey) {
    try {
...
// .wrangler/tmp/bundle-UjKJ4s/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-UjKJ4s/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5799136884439087.js.map
